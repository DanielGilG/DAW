...
-- Ejercicio 1 (nose) /LIMIT + SUBCONSULTAS/
select nombre_cliente
from cliente
group by limite_credito desc
limit 1;
...
-- Ejercicio 2 (ChatGPT)
select p.codigo_producto, min(cantidad_en_stock)
from producto p
group by p.codigo_producto
order by p.cantidad_en_stock asc
limit 1;

-- Ejercicio 3 (recursividad ?? ) /SUBCONSULTAS/
select e1.nombre, e1.apellido1, e1.apellido2, e1.email
from empleado e1
where e1.codigo_jefe = (select e2.codigo_empleado
						from empleado e2
						where e2.nombre = "Alberto" and e2.apellido1 = "Soria");

-- Ejercicio 4 (ni idea de que hace el cod_rep...)
select e1.nombre, e1.apellido1, e1.puesto
from empleado e1
where e1.codigo_empleado not in (select codigo_empleado_rep_ventas
from cliente c1);
...
-- Ejercicio 6 (creo que no pide esto) /SUBCONSULTAS/
select o.codigo_oficina, p.gama
from oficina o, empleado e, cliente c, producto p, pedido pe, detalle_pedido dp
where c.codigo_cliente = pe.codigo_cliente and
pe.codigo_pedido = dp.codigo_pedido and
dp.codigo_producto = p.codigo_producto and
p.gama != "Frutales" and
c.codigo_empleado_rep_ventas != e.codigo_empleado and
e.codigo_oficina = o.codigo_oficina;

		# Oficina de (empleados de (clientes Frutal)
select o1.codigo_oficina
from oficina o1
where o1.codigo_oficina not in (select e2.codigo_oficina
								from empleado e2
								where e2.codigo_empleado in (select c3.codigo_empleado_rep_ventas
															 from cliente c3, pedido p3, detalle_pedido dp3, producto po3
															 where (c3.codigo_cliente = p3.codigo_cliente) and (p3.codigo_pedido = dp3.codigo_pedido) and (dp3.codigo_producto = po3.codigo_producto) and (po3.gama = "Frutales")));

	# Con Join
select distinct(e.codigo_oficina)
from empleado e inner join cliente c on c.codigo_empleado_rep_ventas = e.codigo_empleado
				inner join pedido pe on pe.codigo_cliente = 
;

-- Ejercicio 7 (creo que sii)
select c.nombre_cliente
from cliente c
where c.codigo_cliente not in (select p.codigo_cliente
							   from pago p) and
	  c.codigo_cliente in (select pe.codigo_cliente
						   from pedido pe);

-- Ejercicio 8 (este tambien chill)
select distinct p.nombre
from producto p, pedido pe, detalle_pedido de
where p.codigo_producto = de.codigo_producto and
de.codigo_pedido = pe.codigo_pedido;

-- Ejercicio 9 (falta los de 0 pedidos)
select c.nombre_cliente, count(codigo_pedido)
from cliente c, pedido p
where c.codigo_cliente = p.codigo_cliente
group by c.nombre_cliente;

-- Ejecicio 10 (daleee)
select distinct c.nombre_cliente, year(p.fecha_pedido)
from cliente c, pedido p
