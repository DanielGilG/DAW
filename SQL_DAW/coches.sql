...
-- Ejercicio 3
select con.cifc
from concesionario con, distribucion d
where con.cifc = d.cifc and d.cantidad > 18;

-- Ejercicio 4
select con.cifc
from concesionario con, distribucion d
where con.cifc = d.cifc and d.cantidad >= 10 and d.cantidad <= 18;

-- Ejercicio 6 (para el final)
select c.cifc, co.modelo
from concesionario c, coche co, venta v
where c.cifc = v.cifc and v.codcoche = co.codcoche
group by co.modelo
having count(co.modelo) = 10; 
...
where c.cifc = d.cifc and c.ciudad = "Barcelona";

-- Ejercicio 10
select car.codcoche
from venta v, cliente c, coche car
where v.cifcl = c.cifcl and 
v.codcoche = car.codcoche and 
c.ciudad = "Madrid";